...
function Comp ({ location, dispatch, fazhandangyuanxinxi, form }) {
  const { name = '发展党员信息' } = location.query,
  ...
  const layoutItem = (item, onClick) => {
              ...
              attachments.map((val , index) =>{
                ...
                if(url != '')
                  result.push(
                    ...
                    <div key={index} data-src={url} className='img' style={{backgroundImage:`url(${url})`}}></div>
                  </div>)
              })
            }
            break
    ...
    layoutBtn = () => {
      const result = []
      if (btnText.length > 0) {
        result.push(<div style={{ height: '100px' }}></div>)
        result.push(
          <div className={styles[`${PrefixCls}-button`]}>
            {<WingBlank><Button type="primary" loading={uploading} onClick={handleSubmits}>{btnText}</Button></WingBlank>}
          </div>)
      }
      return result
    }
