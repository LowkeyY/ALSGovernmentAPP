...
const PrefixCls = 'lawnotice';

class LawNotice extends React.Component {
  constructor (props) {
    super(props);
  }
  ...
  render () {
    const { name = '申请指南', } = this.props.location.query,
      { currentData: { content, title, date }, isOpen, viewImages, viewImageIndex } = this.props.lawnotice,
      getContents = () => {
        return {
          __html: content,
        };
...
export default connect(({ loading, lawnotice }) => ({
  loading,
  lawnotice,
}))(LawNotice);
